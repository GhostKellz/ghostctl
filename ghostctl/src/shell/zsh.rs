pub fn install_zsh() {
    ...
    let is_installed = std::process::Command::new("which").arg("zsh").output().map(|o| o.status.success()).unwrap_or(false);
    if is_installed {
    ...
    if !std::path::Path::new(&p10k_dir).exists() {
        let status = std::process::Command::new("git")
            .args(["clone", "--depth=1", "https://github.com/romkatv/powerlevel10k.git", &p10k_dir])
            .status()
    ...
    if let Ok(mut zshrc) = std::fs::OpenOptions::new().append(true).create(true).open(&zshrc_path) {
        use std::io::Write;
