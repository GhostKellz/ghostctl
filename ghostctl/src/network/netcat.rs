...
pub fn receive_file(file_path: &str, port: u16) {
    println!(
        "📥 Listening on port {} for incoming file (will save as {})...",
        port, file_path
    );
...
pub fn chat(host: Option<&str>, port: u16) {
    match host {
        Some(host) => {
            ...
            let _ = Command::new("nc").arg(host).arg(port.to_string()).status();
        }
        None => {
            ...
            let _ = Command::new("nc").arg("-l").arg(port.to_string()).status();
        }
    }
}
